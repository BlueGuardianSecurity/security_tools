#port scanner based off faan ross tutorial. never port scan without permission.
...
def ping_sweep(network, netmask):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        futures = {executor.submit(ping, host): host for hosts in hosts}
...
def scan_port(args):
    ip, port = args
    response = sr1(IP(dst=ip)/TCP(sport=port, flags="S"), timeout=1, verbose=0)
